...
1. Compare each element of array A with every element of array B.
2. If equal, add to result (avoid duplicates).
...
2. Check elements of second array.
3. Add common ones to result.
...
3. Add common elements (avoid duplicates).
...
| ---------------------- | --------------- | ----- | -------------------- |
| Brute                  | O(n×m)          | O(m)  | Very small arrays    |
| Better (set)           | O((n+m) log n)  | O(n)  | When arrays unsorted |
| Optimal (two pointers) | O(n+m)          | O(1)  | Sorted arrays        |
